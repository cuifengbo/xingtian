...
self.onmessage = (event) => {
    ...
    if (data.type === MESSAGE_TYPE_YW) {
        ...
        if(data.payload.type === 'genMap') {
            console.log('生成地图', data.payload.width, data.payload.height, data.payload.seed);
            const map = mapgen.generateMap(data.payload.width, data.payload.height, data.payload.seed);
